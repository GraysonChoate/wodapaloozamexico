...
3. **Preview URL and the local command to run it.** There is no hosted preview; say so
   ...
   who only has GitHub can still see the film. Generated by **`build/audit.sh`**, which
   captures the frames and writes `AUDIT.md` from the same stop list, so the table cannot
   drift from the pictures. It is not hand-maintained and must not become so: the index was
   previously produced by copying the previous milestone's file, the "previous" one was
   picked with an alphabetical `tail -1`, and three indexes ended up describing eighteen
   stops for sets of sixty to eighty frames — with the numbers pointing at the wrong
   pictures. An index that misdescribes the evidence is worse than no index, because it is
